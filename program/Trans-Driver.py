...
class MultiHeadSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # q = self.linear_q(x).reshape(batch, nh, dk).transpose(1, 2)  # (batch, nh, n, dk) 5.reshape(16,5,2)
        # k = self.linear_k(x).reshape(batch, nh, dk).transpose(1, 2)  # (batch, nh, n, dk)
        # v = self.linear_v(x).reshape(batch, nh, dv).transpose(1, 2)  # (batch, nh, n, dv)
...
def train():
    ...
    file_names = ['2020plus']
    # file_names = ['ActiveDriver', 'OncodriveFML', 'HotMAPS', 'CompositeDriver', '2020plus']
    # file_names = ['ActiveDriver', '2020plus', 'e-Driver',
    #  'CompositeDriver',  'CHASM', 'VEST', 'MuSiC', 'OncodriveFML']
    ...
    cal_df = pd.DataFrame(columns=['Cancer', 'ActiveDriver', '2020plus',  'e-Driver',
                                    'CompositeDriver', 'CHASM', 'VEST', 'Trans-Driver', 'DriverNet', 'HotMAPS'])
    len_each = {}
    ...
    for file in cancers:
        ...
        methods = ['ActiveDriver', '2020plus',  'e-Driver',
                    'CompositeDriver',  'CHASM', 'VEST', 'DriverNet', 'Trans-Driver', 'HotMAPS']
        ...
        for method_file in method_dirs:
            ...
            if method_file == '2020plus' or method_file == 'CompositeDriver' or method_file == 'OncodriveCLUST' or method_file=='DriverNet':
                ...
                q = 0.01
            elif method_file == 'MutSig2CV' or method_file == 'MuSiC' or method_file == 'e-Driver' \
